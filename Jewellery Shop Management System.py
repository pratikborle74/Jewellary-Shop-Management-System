...
class JewelleryShop:
    ...
    def save_jewellery_to_csv(self, filename):
        with open(filename, mode='w', newline='') as file:
            ...
            for jewellery in self.jewellery_list:
                writer.writerow([jewellery.name, jewellery.price, jewellery.description])
    ...
    def load_jewellery_from_csv(self, filename):
        ...
        with open(filename, mode='r') as file:
            ...
            for row in reader:
                if len(row) >= 3:  # Ensure at least three values are present (name, price, description)
                    name, price, description = row
                    jewellery = Jewellery(name, float(price), description)
                    self.jewellery_list.append(jewellery)

class Sale:
    def __init__(self, jewellery, quantity, customer, date):
            ...
            for row in reader:
                if len(row) >= 6:  # Ensure all six values are present
                    ...
                    if jewellery and customer:
                        sale = Sale(jewellery, int(quantity), customer, date)
                        self.sales_list.append(sale)
...
def load_customers_from_csv(customers):
    try:
        ...
        print("Customers file not found. Starting with an empty customer list.")
...
def main():
    ...
    load_customers_from_csv(customers)  # Load customers here
    ...
    while True:
        ...
        if choice == '1':
            # Customer Purchase
            customer_purchase(shop, sales_management, customers)
        elif choice == '2':
            # Admin Login
            ...
            shop.save_jewellery_to_csv("jewellery_data.csv")
            sales_management.save_sales_to_csv("sales_data.csv")
            save_customers_to_csv(customers)  # Save customers here
            break
        else:
            print("Invalid choice. Please select a valid option.")
